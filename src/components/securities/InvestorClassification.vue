  ...
  try {
    ...
    if (response.data) {
      let result = response.data.items.map((item: any) => {
        ...
        total.value.localInstitutionalNetValueMatched += parseCurrency(
          item.localInstitutionalNetValueMatched
        );
  ...
  <div class="card">
    ...
    <div v-if="transactionData.length != 0" class="ms-2">
      ...
      <ul>
        <li>
          <span
            >Cá nhân khớp lệnh ròng: {{ formatNumber(total.localIndividualNetValueMatched) }}</span
          >
        </li>
        <li>
          <span
            >Tổ chức khớp lệnh ròng:
            {{ formatNumber(total.localInstitutionalNetValueMatched) }}</span
          >
        </li>
        <li>
          <span
